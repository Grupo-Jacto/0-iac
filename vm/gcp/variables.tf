...
variable "vm_firewall_name" {
  ...
  default     = "my-firewall-for-vm"
  validation {
    condition     = var.vm_firewall_name == null || length(var.vm_firewall_name) > 0 || can(regex("^[a-z0-9][a-z0-9_-]{2,25}[a-z0-9]$"))
    error_message = "Nome para a regra de firewall não cumpre os requisitos: \n- letras minúsculas\n- número\n- hifén. "
  }
}
